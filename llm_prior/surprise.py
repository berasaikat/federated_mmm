...
def compute_surprise(
    ...
    for ch, prior_params in prior_dict.items():
        ...
        if None in (mu_prior, sigma_prior, mu_post, sigma_post):
            logger.warning(f"Skipping surprise for '{ch}' — missing parameters. " f"prior={prior_params}, posterior={post_params}")
            continue
        ...
        kl = math.log(sigma_prior / sigma_post) + (var_post + (mu_post - mu_prior)**2) / (2 * var_prior) - 0.5
        kl = max(0.0, kl)
